...
function RatioBars({ foreign, institution }: { foreign: number; institution: number }) {
  ...
  return (
    <div className="flex gap-0.5 w-14 h-2 rounded overflow-hidden shrink-0">
      <div
        className="bg-cyan-brand/70"
        style={{ width: `${fPct}%` }}
        title={`외국인 ${fPct}%`}
      />
      <div
        className="bg-signal-green/70"
        style={{ width: `${oPct}%` }}
        title={`기관 ${oPct}%`}
      />
    </div>
    ...
    <>
      <div className="bg-navy-card border border-navy-border rounded-2xl p-4">
        ...
        <p className="text-xs text-gray-500 mb-3">
          외국인·기관 동반 순매수 TOP 10
        </p>

        {/* Legend */}
        <div className="flex gap-3 mb-2">
          <span className="flex items-center gap-1 text-xs text-gray-500">
            <span className="w-2 h-2 rounded-sm bg-cyan-brand/70 inline-block" />
            외국인
          </span>
          <span className="flex items-center gap-1 text-xs text-gray-500">
            <span className="w-2 h-2 rounded-sm bg-signal-green/70 inline-block" />
            기관
          </span>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-xs text-gray-500 py-6 text-center">
            현재 시장에 강력한 쌍끌이 수급이 유입되는 다른 종목이 없습니다.
          </p>
        ) : (
          <div
            className="max-h-[360px] overflow-y-auto space-y-0 scrollbar-thin"
          ...
          >
            <div className="divide-y divide-navy-border/30">
              {data.stocks.map((stock, idx) => (
                <button
                  key={stock.code}
                  onClick={() => setDrawer({ code: stock.code, market: "KS", name: stock.name })}
                ...
                >
                  ...
                  {/* Change % */}
                  <span
                    className={`text-xs font-semibold num shrink-0 ${
        ...
        {/* Timestamp */}
        {ts && (
          <p className="text-xs text-gray-600 text-right mt-3 border-t border-navy-border/40 pt-2">
            {ts} 기준
          </p>
        )}
      </div>
